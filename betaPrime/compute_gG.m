function [g, G] = compute_gG(y_t, C, mu_tmp, P_tmp, alpha, beta)
...
% M  : 2x2 covariance of Gaussian prior
    ...
    v0 = p(U, alpha, beta);    % p_v(u)
    v1 = dp(U, alpha, beta);   % derivative wrt u
    v2 = ddp(U, alpha, beta);  % second derivative wrt u
    ...
    function y = p(u, alpha, beta)
        % Beta prime pdf, support u > 0
        y = zeros(size(u));
        m = (u > 0);
        B_ab = beta_func(alpha, beta);  % custom helper below
        y(m) = (u(m).^(alpha-1)) .* (1+u(m)).^(-(alpha+beta)) ./ B_ab;
    end
    
    function y = dp(u, alpha, beta)
        % First derivative wrt u
        y = zeros(size(u));
        m = (u > 0);
        pu = p(u(m), alpha, beta);
        y(m) = pu .* ( (alpha-1)./u(m) - (alpha+beta)./(1+u(m)) );
    end
    
    function y = ddp(u, alpha, beta)
        % Second derivative wrt u
        y = zeros(size(u));
        m = (u > 0);
        pu = p(u(m), alpha, beta);
        l1 = (alpha-1)./u(m) - (alpha+beta)./(1+u(m));
        l2 = -(alpha-1)./(u(m).^2) + (alpha+beta)./((1+u(m)).^2);
        y(m) = pu .* ( l1.^2 + l2 );
    end
