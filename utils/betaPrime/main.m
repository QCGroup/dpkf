...
mu_kf(:, 1) = mu0;
P_kf{1} = P0;
R = alpha*(alpha + beta - 1) / (beta - 1)^2 / (beta -2);
muV = alpha / (beta - 1);
...
for trialIter = 1:numTrials
    ...
    for t = 1:N
        ...
        tic;
        [mu_proposed(:, t+1), P_proposed{t+1}] = poposedEstimator(mu_proposed(:, t), P_proposed{t}, alpha, beta, C, A, y(:, t+1), Q);
        time_prposed{trialIter, 1}(t) = toc;
                                                                   
        % Solution using particle filter
        tic;
        predict(pf, numParticles, A, sqrtQ);
        correct(pf, y(:, t+1), C, alpha, beta);
...
%         tic;
%         [mu_mas(:, t+1), P_mas{t+1}] = Masreliez(A, C, Q, y(:, t+1), alpha, beta, mu_mas(:, t), P_mas{t}, 0);
...
% ylabel('RMSE');
% xlabel('Time step');
% lgd = legend('Proposed', 'KF', 'MCKF', 'PF');
% lgd.Box = "off";
% lgd.ItemTokenSize = [12, 12, 12, 12];
% lgd.Location = "northoutside";
% lgd.Orientation = "horizontal";
% exportgraphics(f, 'fig_betaPrime.pdf', 'ContentType','vector');
...
% % disp(['Computation time of the Masreliez has mean ', num2str(time_mas_mean(end)), ' and std ', num2str(time_mas_std(end)), '.']);
% disp(['Computation time of the Proposed method has mean ', num2str(time_proposed_mean(end)), ' and std ', num2str(time_proposed_std(end)), '.']);
% disp(['Computation time of the KF method has mean ', num2str(time_kf_mean(end)), ' and std ', num2str(time_kf_std(end)), '.']);
% disp(['Computation time of the MCKF method has mean ', num2str(time_mckf_mean(end)), ' and std ', num2str(time_mckf_std(end)), '.']);
% disp(['Computation time of the PF method has mean ', num2str(time_pf_mean(end)), ' and std ', num2str(time_pf_std(end)), '.']);
% fprintf('\n')
% % disp(['our explicit method is ', num2str(time_mas_mean(end) / time_proposed_mean(end)), ' times faster!']);
...
function out = LikelihoodFcs(predictedParticels, y_t, C, alpha, beta)
    predictedMeas = C*predictedParticels;
    measurementError = y_t - predictedMeas;
    measurementError(measurementError < 0) = 0;

    out = measurementError.^(alpha - 1).*(measurementError + 1).^(-alpha - beta) ./ beta_func(alpha, beta);
end
function B = beta_func(a,b)
        % Beta function using gammaln for stability
