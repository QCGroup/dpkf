...
%         [mu_kf(:, t+1), P_kf{t+1}] = KF(mu_kf(:, t), P_kf{t}, muV, y(:, t+1), Q, R, A, C);
        ...
        tic;
        [mu_proposed(:, t+1), P_proposed{t+1}] = poposedEstimator(mu_proposed(:, t), P_proposed{t}, mu, c, C, A, y(:, t+1), Q);
        time_prposed{trialIter, 1}(t) = toc;
                                                                   
        % Solution using particle filter
        tic;
        predict(pf, numParticles, A, sqrtQ);
        correct(pf, y(:, t+1), C, mu, c);
...
%         tic;
%         [mu_mas(:, t+1), P_mas{t+1}] = Masreliez(A, C, Q, y(:, t+1), mu, c, mu_mas(:, t), P_mas{t}, 0);
...
% exportgraphics(f, 'fig_levy.pdf', 'ContentType','vector');
...
% disp(['Computation time of Proposed is ', num2str(mean(time_proposed_mean) / mean(time_proposed_mean))]);
% disp(['Computation time of PF is ', num2str(mean(time_pf_mean) / mean(time_proposed_mean))]);
% fprintf('\n')
...
function out = LikelihoodFcs(predictedParticels, y_t, C, mu, c)

    rShift = y_t - C*predictedParticels - mu;
    ok = rShift > 0;
    logL = -inf(size(rShift));
    logL(ok) = 0.5*log(c/(2*pi)) - 1.5*log(rShift(ok)) - c./(2*rShift(ok));
    m = max(logL);  
    if isfinite(m)
        L = exp(logL - m);
    else
        % all particles impossible under Levy; return tiny uniform likelihoods
        L = eps * ones(size(rShift));
    end
    out = L;
end