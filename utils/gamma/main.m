...
for trialIter = 1:numTrials
    ...
    for t = 1:N
        ...
        tic;
        [mu_proposed(:, t+1), P_proposed{t+1}] = poposedEstimator(mu_proposed(:, t), P_proposed{t}, alpha, theta, C, A, y(:, t+1), Q);
        time_prposed{trialIter, 1}(t) = toc;
                                                                   
        % Solution using particle filter
        tic;
        predict(pf, numParticles, A, sqrtQ);
        correct(pf, y(:, t+1), C, alpha, theta);
...
%         tic;
%         [mu_mas(:, t+1), P_mas{t+1}] = Masreliez(A, C, Q, y(:, t+1), alpha, theta, mu_mas(:, t), P_mas{t}, 0);
...
% xlabel('Time step');
% lgd = legend('Bellman', 'KF', 'MCKF', 'PF');
% lgd.Box = "off";
% lgd.ItemTokenSize = [4, 12, 12, 12];
% lgd.Location = "northoutside";
% lgd.Orientation = "horizontal";
% exportgraphics(f, 'fig_gamma_.pdf', 'ContentType','vector');
...
function out = LikelihoodFcs(predictedParticels, y_t, C, alpha, theta)
    predictedMeas = C*predictedParticels;
    measurementError = y_t - predictedMeas;
    measurementError(measurementError < 0) = 0;

    out = 1/(theta^alpha * gamma(alpha)) * measurementError.^(alpha - 1) .* exp(-measurementError ./ theta);
end