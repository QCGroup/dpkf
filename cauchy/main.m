...
%         [mu_kf(:, t+1), P_kf{t+1}] = KF(mu_kf(:, t), P_kf{t}, muV, y(:, t+1), Q, R, A, C);
        ...
        tic;
        [mu_proposed(:, t+1), P_proposed{t+1}] = poposedEstimator(mu_proposed(:, t), P_proposed{t}, gamma, C, A, y(:, t+1), Q);
        time_prposed{trialIter, 1}(t) = toc;
                                                                   
        % Solution using particle filter
        tic;
        predict(pf, numParticles, A, sqrtQ);
        correct(pf, y(:, t+1), C, gamma);
...
%         [mu_mckf(:, t+1), P_mckf{t+1}, ~] = MCKF(A, mu_mckf(:, t), P_mckf{t}, C, y(:, t+1), Q, R, muV, kernel, epsilon);
        ...
        tic;
        [mu_mas(:, t+1), P_mas{t+1}] = Masreliez(A, C, Q, y(:, t+1), gamma, mu_mas(:, t), P_mas{t}, 0);
...
% exportgraphics(f, 'fig_experiment5_rmse_.pdf', 'ContentType','vector');
...
% disp(['Computation time of Masreliez is ', num2str(mean(time_mas_mean) / mean(time_proposed_mean))]);
...
function out = LikelihoodFcs(predictedParticels, y_t, C, gamma)
    predictedMeas = C*predictedParticels;
    measurementError = y_t - predictedMeas;

    out = 1 ./ pi ./ gamma ./ (1 + (measurementError./gamma).^2);
end