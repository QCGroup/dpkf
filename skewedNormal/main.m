...
% Measurement noise (skewed Gaussian with scale=1 and location=0)
alpha = 3;
scale = 2.6505;
% scale = 1.185332;
...
mu_proposed(:, 1) = mu0;
P_proposed{1} = P0;
skewnorm_pdf = @(x) 2/scale * normpdf(x, 0, scale) .* normcdf(alpha * x / scale);
ve = fminbnd(@(x) -skewnorm_pdf(x), -10*scale, 10*scale);

% Kalman filter initialization
mu_kf(:, 1) = mu0;
P_kf{1} = P0;
del = alpha / sqrt(1 + alpha^2);
R = scale^2*(1 - 2*del^2/pi);
muV = scale*sqrt(2/pi)*del;
...
for trialIter = 1:numTrials
    ...
    % Measurement noise
    v = skew_normal_rnd(0, scale, alpha, N+1);
    ...
    for t = 1:N
        ...
        tic;
        [mu_proposed(:, t+1), P_proposed{t+1}] = poposedEstimator(mu_proposed(:, t), P_proposed{t}, alpha, scale, C, A, y(:, t+1), Q, ve);
        time_prposed{trialIter, 1}(t) = toc;
                                                                   
        % Solution using particle filter
        tic;
        predict(pf, numParticles, A, sqrtQ);
        correct(pf, y(:, t+1), C, alpha, scale);
        ...
        tic;
        [mu_mas(:, t+1), P_mas{t+1}] = Masreliez(A, C, Q, y(:, t+1), alpha, scale, mu_mas(:, t), P_mas{t}, 0);
...
% exportgraphics(f, 'fig_skewedNormal.pdf', 'ContentType','vector');
...
function out = LikelihoodFcs(predictedParticels, y_t, C, alpha, scale)
    predictedMeas = C*predictedParticels;
    measurementError = y_t - predictedMeas;
    measurementError = measurementError ./ scale;
    
    phi = 1/sqrt(2*pi).*exp(-measurementError.^2/2);
    Phi = 0.5 * (1 + erf(alpha*measurementError./sqrt(2)));
    out = 2/scale.*phi.*Phi;
end